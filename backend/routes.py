...
def get_pictures():
    # Return the list of pictures (URLs) from the data
    if data:
        return jsonify(data), 200  # Return JSON response with a 200 OK status
    else:
        return jsonify({"message": "No pictures found"}), 404  # If no data is found, return 404 with a message
